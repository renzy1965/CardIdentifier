...
def load_model():
    ...
    model_paths = [
        ...
        './models/best_model.keras(1)',  # ./models directory
        '/content/best_model.keras(1)',  # Colab path
        os.path.join(os.path.dirname(__file__), 'best_model.keras(1)')  # Relative to script
    ]

    # Try loading from local paths
    ...
    # If local loading fails, provide guidance
    st.error("""
    ⚠️ Model file not found. 
    
    Please ensure:
    1. You have downloaded the model file
    2. The model is named 'best_model.keras(1)'
    3. The model is in the same directory as this script
    
    If you don't have the model, you'll need to train or obtain it first.
    """)
    return None
