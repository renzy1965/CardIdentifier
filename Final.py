...
def load_model():
    try:
        ...
        if not os.path.exists(model_path):
            st.error("⚠️ Model file not found. Please ensure 'best_model.keras' is in the same directory.")
            return None
...
def get_class_names(model):
    try:
        return list(model.get_config()['layers'][-1]['config']['units'][0])
    except:
        # Fallback to manual list if automatic extraction fails
        return [
            'Ace of Clubs', 'Ace of Diamonds', 'Ace of Hearts', 'Ace of Spades', 
            # Add all 53 card names here
            # This is a placeholder - you should replace with actual class names from your dataset
        ]
